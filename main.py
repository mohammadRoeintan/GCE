...
parser.add_argument('--validation', action='store_true', help='Enable validation split and evaluation')
...
def plot_metrics(history):
    ...
    has_validation = 'valid_loss' in history and history['valid_loss']
    ...
    if has_validation:
        ...
        plt.plot(epochs, history['test_loss'], 'go-', label='Test Loss')
    ...
    if has_validation:
        ...
        plt.plot(epochs, history['test_hit'], 'gs-', label='Test Recall@20')
        plt.plot(epochs, history['test_mrr'], 'yd-', label='Test MRR@20')
...
def main():
    ...
    if opt.dataset == 'diginetica':
        ...
        num_node = 310 # Fallback for other datasets
    ...
    valid_loader = None
    if opt.validation:
        print("Validation mode is enabled. Splitting train data.")
        train_data, valid_data_raw = split_validation(train_data, opt.valid_portion)
        valid_data = Data(valid_data_raw)
        ...
        print("Validation mode is disabled. Using full train set and evaluating on test set.")
    ...
    if opt.mode == 'train':
        ...
        history = {
            'train_loss': [], 'train_hit': [], 'train_mrr': [],
            'valid_loss': [], 'valid_hit': [], 'valid_mrr': [],
        ...
        for epoch in range(opt.epoch):
            ...
            # Note: Evaluating on the full training set can be slow.
            ...
            if opt.validation:
                ...
                eval_hit, eval_mrr = valid_hit, valid_mrr
            else: # Evaluate on test set if no validation
                test_loss, test_hit, test_mrr = evaluate(model, test_loader)
                history['test_loss'].append(test_loss)
                history['test_hit'].append(test_hit)
                history['test_mrr'].append(test_mrr)
                print(f"Test Results:  Loss: {test_loss:.4f}, Recall@20: {test_hit:.4f}%, MRR@20: {test_mrr:.4f}%")
                eval_hit, eval_mrr = test_hit, test_mrr
            ...
            print(f'Best {"Validation" if opt.validation else "Test"} Result:')
        ...
        # --- Final Test Evaluation (if validation was on) ---
        if opt.validation:
